...
namespace Aplakhtiy.TestForm.Helpers
{
    public static class CustomHelpers
    {
        ...
        {
            //MvcHtmlString label =
            //    LabelExtensions.LabelFor(helper, expression, new { @class = "control-label col-md-2" });
            MvcHtmlString editor =
                EditorExtensions.EditorFor(helper, expression, new { htmlAttributes = new { @class = "form-control" } });
            //MvcHtmlString validation =
            //    ValidationExtensions.ValidationMessageFor(helper, expression, null, new { @class = "text-danger" });

            // Build the input elements
            TagBuilder editorDiv = new TagBuilder("div");
            editorDiv.AddCssClass("col-md-10");
            editorDiv.InnerHtml = editor.ToString();

            // Build the validation message elements
            //TagBuilder validationSpan = new TagBuilder("span");
            //validationSpan.AddCssClass("text-danger field-validation-error");
